...
namespace FriendOrganize.UI.ViewModel
{
    public abstract class DetailViewModelBase : ViewModelBase, IDetailViewModel
    {
        ...
        protected async Task SaveWithOptimisticConcurrencyAsync(Func<Task> saveFunc, Action afferSaveAction)
        {
            try
            {
                await saveFunc();
            ...
            catch (DbUpdateConcurrencyException ex)
            {
                ...
                var result = await MessageDialogService.ShowOkCancelDialogAsync("The entity has been changed in "
                    ...
                    + "to reload the entity from the databse.", "Question");

                if (result == MessageDialogResult.OK)
                {
                    // Update the orginal values with database-value
